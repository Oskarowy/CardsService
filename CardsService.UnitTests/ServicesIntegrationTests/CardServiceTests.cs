...
namespace CardsService.Tests.ServicesIntegrationTests
{
    public class CardServiceTests
    {
        ...
        #endregion
        #region Action6, Action7 - PIN dependable     
        #region Action6 only
        [Theory]
        [InlineData(CardType.Prepaid, CardStatus.Ordered, false, "Card11", "User1")]
        [InlineData(CardType.Credit, CardStatus.Ordered, false, "Card315", "User3")]
        public async Task ShouldNotReturnAction6_ForAnyCardType_IfCardIsOrdered_AndPinIsNotSet(CardType cardType, CardStatus cardStatus, bool isPinSet, string cardNumber, string userId)
        {
            var cardDetails = await _cardService.GetCardDetails(userId, cardNumber);
            var expectedCardDetails = new CardDetails(cardNumber, cardType, cardStatus, isPinSet);
            var allowedActions = await _cardService.GetCardAllowedActions(userId, cardNumber);
            ...
            Assert.Equal(expectedCardDetails, cardDetails);
            Assert.DoesNotContain("ACTION6", allowedActions);
        }
        [Theory]
        [InlineData(CardType.Debit, CardStatus.Ordered, true, "Card18", "User1")]
        [InlineData(CardType.Debit, CardStatus.Ordered, true, "Card28", "User2")]
        public async Task ShouldReturnAction6_ForEveryCardType_IfCardIsOrdered_AndPinIsSet(CardType cardType, CardStatus cardStatus, bool isPinSet, string cardNumber, string userId)
        {
            var cardDetails = await _cardService.GetCardDetails(userId, cardNumber);
            var expectedCardDetails = new CardDetails(cardNumber, cardType, cardStatus, isPinSet);
            var allowedActions = await _cardService.GetCardAllowedActions(userId, cardNumber);
            ...
            Assert.Equal(expectedCardDetails, cardDetails);
            Assert.Contains("ACTION6", allowedActions);
        }
        [Theory]
        [InlineData(CardType.Prepaid, CardStatus.Active, false, "Card13", "User1")]
        [InlineData(CardType.Credit, CardStatus.Active, false, "Card317", "User3")]
        public async Task ShouldNotReturnAction6_ForAnyCardType_IfCardIsActive_AndPinIsNotSet(CardType cardType, CardStatus cardStatus, bool isPinSet, string cardNumber, string userId)
        {
            var cardDetails = await _cardService.GetCardDetails(userId, cardNumber);
            var expectedCardDetails = new CardDetails(cardNumber, cardType, cardStatus, isPinSet);
            var allowedActions = await _cardService.GetCardAllowedActions(userId, cardNumber);
        ...
        [Theory]
        [InlineData(CardType.Debit, CardStatus.Active, true, "Card110", "User1")]
        [InlineData(CardType.Debit, CardStatus.Active, true, "Card210", "User2")]
        public async Task ShouldReturnAction6_ForEveryCardType_IfCardIsActive_AndPinIsSet(CardType cardType, CardStatus cardStatus, bool isPinSet, string cardNumber, string userId)
        {
            var cardDetails = await _cardService.GetCardDetails(userId, cardNumber);
            var expectedCardDetails = new CardDetails(cardNumber, cardType, cardStatus, isPinSet);
            var allowedActions = await _cardService.GetCardAllowedActions(userId, cardNumber);
        ...
        [Theory]
        [InlineData(CardType.Debit, CardStatus.Inactive, false, "Card29", "User2")]
        [InlineData(CardType.Debit, CardStatus.Inactive, false, "Card19", "User1")]
        public async Task ShouldNotReturnAction6_ForAnyCardType_IfCardIsInactive_AndPinIsNotSet(CardType cardType, CardStatus cardStatus, bool isPinSet, string cardNumber, string userId)
        {
            var cardDetails = await _cardService.GetCardDetails(userId, cardNumber);
            var expectedCardDetails = new CardDetails(cardNumber, cardType, cardStatus, isPinSet);
            var allowedActions = await _cardService.GetCardAllowedActions(userId, cardNumber);
        ...
        [InlineData(CardType.Prepaid, CardStatus.Inactive, true, "Card32", "User3")]
        [InlineData(CardType.Credit, CardStatus.Inactive, true, "Card116", "User1")]
        public async Task ShouldReturnAction6_ForEveryCardType_IfCardIsInactive_AndPinIsSet(CardType cardType, CardStatus cardStatus, bool isPinSet, string cardNumber, string userId)
        {
            var cardDetails = await _cardService.GetCardDetails(userId, cardNumber);
            var expectedCardDetails = new CardDetails(cardNumber, cardType, cardStatus, isPinSet);
            var allowedActions = await _cardService.GetCardAllowedActions(userId, cardNumber);
        ...
        [InlineData(CardType.Prepaid, CardStatus.Ordered, false, "Card11", "User1")]
        [InlineData(CardType.Credit, CardStatus.Ordered, false, "Card315", "User3")]
        public async Task ShouldReturnAction7_ForEveryCardType_IfCardIsOrdered_AndPinIsNotSet(CardType cardType, CardStatus cardStatus, bool isPinSet, string cardNumber, string userId)
        {
            var cardDetails = await _cardService.GetCardDetails(userId, cardNumber);
            var expectedCardDetails = new CardDetails(cardNumber, cardType, cardStatus, isPinSet);
            var allowedActions = await _cardService.GetCardAllowedActions(userId, cardNumber);
            ...
            Assert.Equal(expectedCardDetails, cardDetails);
            Assert.Contains("ACTION7", allowedActions);
        }
        [Theory]
        [InlineData(CardType.Debit, CardStatus.Ordered, true, "Card18", "User1")]
        [InlineData(CardType.Debit, CardStatus.Ordered, true, "Card28", "User2")]
        public async Task ShouldNotReturnAction7_ForAnyCardType_IfCardIsOrdered_AndPinIsSet(CardType cardType, CardStatus cardStatus, bool isPinSet, string cardNumber, string userId)
        {
            var cardDetails = await _cardService.GetCardDetails(userId, cardNumber);
            var expectedCardDetails = new CardDetails(cardNumber, cardType, cardStatus, isPinSet);
            var allowedActions = await _cardService.GetCardAllowedActions(userId, cardNumber);
            ...
            Assert.Equal(expectedCardDetails, cardDetails);
            Assert.DoesNotContain("ACTION7", allowedActions);
        }
        [Theory]
        [InlineData(CardType.Prepaid, CardStatus.Active, false, "Card13", "User1")]
        [InlineData(CardType.Credit, CardStatus.Active, false, "Card317", "User3")]
        public async Task ShouldReturnAction7_ForEveryCardType_IfCardIsActive_AndPinIsNotSet(CardType cardType, CardStatus cardStatus, bool isPinSet, string cardNumber, string userId)
        {
            var cardDetails = await _cardService.GetCardDetails(userId, cardNumber);
            var expectedCardDetails = new CardDetails(cardNumber, cardType, cardStatus, isPinSet);
            var allowedActions = await _cardService.GetCardAllowedActions(userId, cardNumber);
        ...
        [InlineData(CardType.Debit, CardStatus.Active, true, "Card110", "User1")]
        [InlineData(CardType.Debit, CardStatus.Active, true, "Card210", "User2")]
        public async Task ShouldNotReturnAction7_ForAnyCardType_IfCardIsActive_AndPinIsSet(CardType cardType, CardStatus cardStatus, bool isPinSet, string cardNumber, string userId)
        {
            var cardDetails = await _cardService.GetCardDetails(userId, cardNumber);
            var expectedCardDetails = new CardDetails(cardNumber, cardType, cardStatus, isPinSet);
            var allowedActions = await _cardService.GetCardAllowedActions(userId, cardNumber);
        ...
        [InlineData(CardType.Debit, CardStatus.Inactive, false, "Card29", "User2")]
        [InlineData(CardType.Debit, CardStatus.Inactive, false, "Card19", "User1")]
        public async Task ShouldReturnAction7_ForEveryCardType_IfCardIsInactive_AndPinIsNotSet(CardType cardType, CardStatus cardStatus, bool isPinSet, string cardNumber, string userId)
        {
            var cardDetails = await _cardService.GetCardDetails(userId, cardNumber);
            var expectedCardDetails = new CardDetails(cardNumber, cardType, cardStatus, isPinSet);
            var allowedActions = await _cardService.GetCardAllowedActions(userId, cardNumber);
        ...
        [InlineData(CardType.Prepaid, CardStatus.Inactive, true, "Card32", "User3")]
        [InlineData(CardType.Credit, CardStatus.Inactive, true, "Card116", "User1")]
        public async Task ShouldNotReturnAction7_ForAnyCardType_IfCardIsInactive_AndPinIsSet(CardType cardType, CardStatus cardStatus, bool isPinSet, string cardNumber, string userId)
        {
            var cardDetails = await _cardService.GetCardDetails(userId, cardNumber);
            var expectedCardDetails = new CardDetails(cardNumber, cardType, cardStatus, isPinSet);
            var allowedActions = await _cardService.GetCardAllowedActions(userId, cardNumber);
        ...
        #endregion
        #region Both Action6 and Action7
        [Theory]
        [InlineData(CardType.Debit, CardStatus.Restricted, false, "Card211", "User2")]
        [InlineData(CardType.Credit, CardStatus.Restricted, true, "Card218", "User2")]
        public async Task ShouldNotReturnAction6and7_ForAnyCardType_IfCardIsRestricted(CardType cardType, CardStatus cardStatus, bool isPinSet, string cardNumber, string userId)
        {
            var cardDetails = await _cardService.GetCardDetails(userId, cardNumber);
            var expectedCardDetails = new CardDetails(cardNumber, cardType, cardStatus, isPinSet);
            var allowedActions = await _cardService.GetCardAllowedActions(userId, cardNumber);
        ...
        public async Task ShouldNotReturnAction6and7_ForAnyCardType_IfCardIsExpired(CardType cardType, CardStatus cardStatus, bool isPinSet, string cardNumber, string userId)
        {
            var cardDetails = await _cardService.GetCardDetails(userId, cardNumber);
            var expectedCardDetails = new CardDetails(cardNumber, cardType, cardStatus, isPinSet);
            var allowedActions = await _cardService.GetCardAllowedActions(userId, cardNumber);
        ...
        [InlineData(CardType.Debit, CardStatus.Closed, true, "Card314", "User3")]
        public async Task ShouldNotReturnAction6and7_ForAnyCardType_IfCardIsClosed(CardType cardType, CardStatus cardStatus, bool isPinSet, string cardNumber, string userId)
        {
            var cardDetails = await _cardService.GetCardDetails(userId, cardNumber);
            var expectedCardDetails = new CardDetails(cardNumber, cardType, cardStatus, isPinSet);
            var allowedActions = await _cardService.GetCardAllowedActions(userId, cardNumber);
        ...
        [Theory]
        [InlineData(CardType.Debit, CardStatus.Blocked, true, "Card312", "User3")]
        [InlineData(CardType.Debit, CardStatus.Blocked, true, "Card112", "User1")]
        [InlineData(CardType.Debit, CardStatus.Blocked, true, "Card212", "User2")]
        public async Task ShouldReturnAction6and7_ForEveryCardType_IfCardIsBlocked_AndPinIsSet(CardType cardType, CardStatus cardStatus, bool isPinSet, string cardNumber, string userId)
        {
            var cardDetails = await _cardService.GetCardDetails(userId, cardNumber);
            var expectedCardDetails = new CardDetails(cardNumber, cardType, cardStatus, isPinSet);
            var allowedActions = await _cardService.GetCardAllowedActions(userId, cardNumber);
        ...
        [InlineData(CardType.Prepaid, CardStatus.Blocked, false, "Card25", "User2")]
        [InlineData(CardType.Credit, CardStatus.Blocked, false, "Card119", "User1")]
        public async Task ShouldNotReturnAction6and7_ForAnyCardType_IfCardIsBlocked_AndPinIsNotSet(CardType cardType, CardStatus cardStatus, bool isPinSet, string cardNumber, string userId)
        {
            var cardDetails = await _cardService.GetCardDetails(userId, cardNumber);
            var expectedCardDetails = new CardDetails(cardNumber, cardType, cardStatus, isPinSet);
            var allowedActions = await _cardService.GetCardAllowedActions(userId, cardNumber);
